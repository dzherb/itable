<template>
  ...
  >
    <LabeledInput autofocus v-model="email" label="Почта" name="email" />
    <LabeledInput v-model="password" label="Пароль" name="password" type="password" />
...
import LabeledInput from '@/components/reusable/forms/inputs/LabeledInput.vue'
...
const {
  ...
  isLoading,
  isReady: isLoggedIn,
} = useAsyncState(() => useAuthStore().login(email.value, password.value), null, {
  immediate: false,
})
