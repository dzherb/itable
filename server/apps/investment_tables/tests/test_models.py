...
class TableTemplateTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.stock1 = Security.objects.create(ticker='A')
        cls.stock2 = Security.objects.create(ticker='B')
        cls.table_template = TableTemplate.objects.create(
            name='test table',
            slug='test-table',
        )
...
class TableSnapshotTestCase(TestCase):
    ...
    async def test_can_create_snapshot_from_template(self):
        snapshot = await TableSnapshot.from_template(
    ...
    async def test_default_snapshot_item_coefficient(self):
        ...
        snapshot_item: TableSnapshotItem = await snapshot.items.aget(
            template_item__security__ticker='A',
        )
